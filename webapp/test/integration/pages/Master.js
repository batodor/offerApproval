sap.ui.define([
	...
	"OfferApproval/OfferApproval/test/integration/pages/Common",
		...
		onTheMasterPage : {
			...
			actions : {
				...
				iSortTheListOnUnitNumber : function () {
					return this.iChooseASorter("sortButton", "Sort By <Value>");
				},

				iFilterTheListOnUnitNumber : function () {
					return this.iMakeASelection("filterButton", "<Value>", "<100 <ApprovalRequestedOn>");
				},

				iGroupTheList : function () {
					return this.iChooseASorter("groupButton", "<Value> Group");
				},
						...
						success : function (aEntityData) {
							this.waitFor({
								...
								success : function (oList) {
									var sCurrentId,
										aItemsNotInTheList = aEntityData.filter(function (oObject) {
											return !oList.getItems().some(function (oListItem) {
												return oListItem.getBindingContext().getProperty("WorkitemID") === oObject.WorkitemID;
											});
										});

									if (!aItemsNotInTheList.length) {
										// Not enough items all of them are displayed so we take the last one
										sCurrentId = aEntityData[aEntityData.length - 1].WorkitemID;
									} else {
										sCurrentId = aItemsNotInTheList[0].WorkitemID;
									}
									...
									oCurrentItem.bindingPath = "/" + oList.getModel().createKey("Workitems", {
										WorkitemID : sCurrentId
									});
				...
				iRememberTheListItem : function (oListItem) {
					...
					this.getContext().currentItem = {
						...
						id: oBindingContext.getProperty("WorkitemID"),
						title: oBindingContext.getProperty("Customer")
					};
				}
			},
			...
			assertions : {
				...
				theListHeaderDisplaysZeroHits : function () {
					return this.waitFor({
						...
						id: "masterPageTitle",
						autoWait: false,
						matchers: new PropertyStrictEquals({name : "text", value : "<Workitems> (0)"}),
						success: function () {
				...
				theListShouldBeFilteredOnUnitNumber : function () {
					return this.theListShouldBeFilteredOnFieldUsingComparator("Value", 100);
				},


				theListShouldBeSortedAscendingOnUnitNumber : function () {
					return this.theListShouldBeSortedAscendingOnField("Value");
				},

				theListShouldBeSortedAscendingOnName : function () {
					return this.theListShouldBeSortedAscendingOnField("Customer");
				},
				...
				theListShouldHaveAllEntries : function () {
					...
					// retrieve all Workitems to be able to check for the total amount
					...
					return this.waitFor({
						...
						success : function (oList) {
							...
							this.waitFor({
								id : "masterPageTitle",
								viewName : sViewName,
								matchers : new PropertyStrictEquals({name : "text", value : "<Workitems> (" + iExpectedLength + ")"}),
								success : function () {
